...
import static com.deeplake.genshin12.util.EntityUtil.EnumFaction.CRITTER;
import static com.deeplake.genshin12.util.EntityUtil.EnumFaction.PLAYER;
import static com.deeplake.genshin12.util.NBTStrDef.IDLNBTDef.KEY_FACTION;
...
//            Idealland.Log("EnumFaction ID = %s", ((ICustomFaction) this).getFaction());
    ...
    public void onUpdate() {
        ...
        if (EntityUtil.getAttitude(PLAYER, this) == EntityUtil.EnumAttitude.HATE) {
...
//        mecPotionSet.add(ModPotions.NOTICED_BY_MOR);
    ...
    protected boolean processInteract(EntityPlayer player, EnumHand hand) {
        if (this instanceof ICustomFaction) {
            if (!player.getEntityWorld().isRemote && player.isCreative()) {
                ...
                if (stack.getItem() == ModItems.DEBUG) {
                    ...
                    faction.setFaction(PLAYER);
                } else if (stack.getItem() == ModItems.DEBUG_2) {
    ...
    public static void onBlockBreak(BlockEvent.BreakEvent event) {
        ...
        List<EntityModUnit> units = EntityUtil.getEntitiesWithinAABB(world, EntityModUnit.class, new Vec3d(event.getPos().up()), 0.5f, null);
