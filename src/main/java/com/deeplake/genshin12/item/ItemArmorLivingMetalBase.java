...
import static com.deeplake.genshin12.util.NBTStrDef.IDLNBTDef.BIOMETAL_WARNED;
		...
		if (!worldIn.isRemote)
        {
            if (((float)stack.getItemDamage() / stack.getMaxDamage()) >= 0.9f)
            {
                if (worldIn.getWorldTime() % TICK_PER_SECOND == 0 && !IDLNBTUtil.GetBoolean(stack, BIOMETAL_WARNED, false)) {
                    ...
                    CommonFunctions.SendMsgToPlayer((EntityPlayerMP) player, "genshin12.msg.low_dura", stack.getDisplayName());
                }
            }
            else {
...
//
//        //tooltip.add(I18n.format("genshin12.gua_enhance_total.desc", IDLSkillNBT.GetGuaEnhanceTotal(stack)));
